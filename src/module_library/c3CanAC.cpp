    ...
    int const nlayers)
{
    ...
    auto leaf_photo =
        ...
        // Ball-Berry conductance cannot be less than its water-stress-adjusted
        // intercept, so this provides a physical lower bound.
        ...
        // Increase the upper bound until the residual changes sign. A failure
        // to find such a bracket is reported separately from solver failure.
        ...
        root_finding::result_t result =
            dekker_solver.solve(gs_residual, gs_lower, gs_upper);
        ...
        if (!root_finding::is_successful(result.flag)) {
            throw std::runtime_error(
                "c3Canopy conductance solvers failed. Termination flag:\n    " +
